...
public class FeedProcessor {
  ...
  public void processTweets() {
    KafkaClient<String, TwitterFeed> kafkaConsumer =
        new KafkaClient<>(getKafkaClusterPropsForIngestion());
    KafkaClient<String, TwitterFeed> kafkaPublisher =
        new KafkaClient<>(getKafkaClusterPropsForIngestion());
    ...
    kafkaConsumer
        ...
        .forEach(
            e -> {
              // There are tweets which are deleted by the user meaning id is missing, which we are
              // conveniently ignoring for now.
